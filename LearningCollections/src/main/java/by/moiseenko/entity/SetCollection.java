...
public class SetCollection {
  ...
  public static void doAction(boolean isActive) {
    if (isActive) {
      ...
      LOG.debug("************************NETWORK USER SET************************");

      Set<NetworkUser> networkUserSet = new HashSet<>(NetworkUserStorage.loadNetworkUsers());
      LOG.debug(networkUserSet.size());
      TreeSet<NetworkUser> networkUserTreeSet =
          new TreeSet<>(new NetworkUserComparator(NetworkUserEnum.AGE, true));
      networkUserTreeSet.addAll(networkUserSet);
      for (NetworkUser n : networkUserTreeSet) {
        LOG.debug(n);
      }
      LOG.debug(networkUserTreeSet.size());
      LOG.debug("****************************************************************");

      EnumSet<NetworkUserEnum> networkUserEnumSet = EnumSet.range(NetworkUserEnum.ID, NetworkUserEnum.GENDER);
      LOG.debug("enums in range ID - GENDER");
      for (NetworkUserEnum nue : networkUserEnumSet) {
        LOG.debug(nue);
      }
      LOG.debug("enum that complementOf set below");
      EnumSet<NetworkUserEnum> othetNetworkUserEnumSet = EnumSet.complementOf(networkUserEnumSet);
      for (NetworkUserEnum nue : othetNetworkUserEnumSet) {
        LOG.debug(nue);
      }
